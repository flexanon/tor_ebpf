...
uint64_t consider_sending_sendme(relay_process_edge_t *pedge) {
  ...
  if (pedge->layer_hint)
    layer_hint_del_window = (int) get(RELAY_LAYER_HINT_DELIVER_WINDOW, 1, layer_hint);
  else
    circ_del_window = (int) get(RELAY_CIRC_DELIVER_WINDOW, 1, circ);
  ...
  while ((pedge->layer_hint ? layer_hint_del_window : circ_del_window) <=
    ...
    if (layer_hint_del_window) {
      ...
      set(RELAY_LAYER_HINT_DELIVER_WINDOW, 2, layer_hint, layer_hint_del_window);
    }
    else {
